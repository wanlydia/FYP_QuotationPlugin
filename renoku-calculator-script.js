jQuery(document).ready(function($) {
    ...
    $('.bvd-button.kitchen-set1').click(function() {
        ...
        if ($this.hasClass('selected')) { // If the button is already selected
            ...
            $('.bvd-button.kitchen-set1').removeClass('selected'); // Deselect all other buttons in the set
    ...
    $('#bvd-submit').click(function() {
        var inputValue = parseFloat($('#bvd-input').val()); // Get the input value and convert it to a float
        if (isNaN(inputValue)) { // Check if the input is a valid number
            $('#bvd-result').text('Please enter a valid number'); // Display an error message if not
        ...
        if (selectedValueSet1 !== null || selectedValueSet2 !== null) { // Check if at least one button is selected
            ...
            $('#bvd-result').text('Result: $' + result); // Display the result
            window.location.href = 'URL_TO_CALC_LIVING_PAGE'; // Redirect to the calc-living page (replace with actual URL)
        } else {
            $('#bvd-result').text('Please select at least one button'); // Display an error message if no button is selected
        }
    });
});
